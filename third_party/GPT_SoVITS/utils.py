...
def get_hparams(init=True, stage=1):
    ...
    # 确保 text_low_lr_rate 是浮点数类型
    if "train" in config and "text_low_lr_rate" in config["train"]:
        if isinstance(config["train"]["text_low_lr_rate"], str):
            config["train"]["text_low_lr_rate"] = float(config["train"]["text_low_lr_rate"])

...
os.chdir(now_dir)
# 使用 insert(0,...) 确保推理项目路径优先于 venv site-packages 中的原项目路径
...
version = model_version = os.environ.get("version", "v2Pro")
...
from tools.ref_audio_loader import RefAudioLoader

ref_loader = RefAudioLoader()

path_sovits_v3 = "GPT_SoVITS/pretrained_models/s2Gv3.pth"  # v3已删除，此处保留兼容
path_sovits_v4 = "GPT_SoVITS/pretrained_models/gsv-v4-pretrained/s2Gv4.pth"  # v4已删除，此处保留兼容
...
def on_model_change(gpt_path, sovits_path, prompt_language=None, text_language=None):
    """
    模型选择变更回调 - 统一处理加载/卸载逻辑
    ...
    处理逻辑:
      ...
      3. 不同目录 -> 卸载参考音频
    """
    result = ref_loader.load_ref(gpt_path, sovits_path)
    
    audio_update = result['audio']
    text_update = result['text']
    status_update = result['status']
    
    ref_audio_value = audio_update if audio_update else None
    ref_text_value = text_update
...
def on_clear_refs():
    """清空参考音频和文本"""
    ref_loader.unload("用户手动清空")
    return None, "", "📤 已清空参考音频和文本"
...
with gr.Blocks(title="GPT-SoVITS WebUI", analytics_enabled=False, js=js, css=css) as app:
    ...
    with gr.Row():
        with gr.Column():
            ...
            ref_load_status = gr.Markdown(
                value="📋 请选择同一目录下的GPT和SoVITS模型以自动加载参考音频",
                visible=True,
            )
            with gr.Row():
            ...
            with gr.Row():
                clear_refs_btn = gr.Button(i18n("清空参考"), variant="stop", size="sm")
    ...
    with gr.Group():
        ...
        SoVITS_dropdown.change(
            on_model_change,
            [GPT_dropdown, SoVITS_dropdown, prompt_language, text_language],
            [inp_ref, prompt_text, ref_load_status],
        )
        GPT_dropdown.change(
            on_model_change,
            [GPT_dropdown, SoVITS_dropdown, prompt_language, text_language],
        ...
        clear_refs_btn.click(on_clear_refs, [], [inp_ref, prompt_text, ref_load_status])

...
class TTS_Config:
    default_configs = {
        "v2Pro": {
            ...
            "is_half": False,
            "version": "v2Pro",
            "t2s_weights_path": "weights/test/test-e10_final.ckpt",
            "vits_weights_path": "weights/test/test_final.pth",
    ...
    def __init__(self, configs: Union[dict, str] = None):
        ...
        assert self.version == "v2Pro", "Only v2Pro version is supported!"
    ...
    def update_version(self, version: str) -> None:
        self.version = version
        self.languages = self.__class__.languages  # 使用类属性的语言列表
...
class TTS:
    ...
    def _init_models(
        ...
        # CPU 模式下必须显式转 float32，否则 LoRA 训练保存的 float16 权重会与 float32 输入冲突
    ...
    def _get_ref_spec(self, ref_audio_path):
        # 用 librosa 替代 torchaudio.load，避免 torchcodec/FFmpeg 依赖
        audio_np, raw_sr = librosa.load(ref_audio_path, sr=None, mono=False)
        raw_audio = torch.from_numpy(np.asarray(audio_np, dtype=np.float32))
        if raw_audio.dim() == 1:
            raw_audio = raw_audio.unsqueeze(0)  # (1, samples)
    ...
    def run(self, inputs: dict):
            ...
            inputs (dict):
                {
                    ...
                    "text_split_method": "auto",  # str. text split method, see text_segmentation_method.py for details.
        ...
        text_split_method: str = inputs.get("text_split_method", "auto")
